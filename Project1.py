...
def solve_problem(N, K, d):
    ...
    # # Constraint: Continuous trip
    # for k in range(K):
    #     for j in range(1, N + 1):
    #         continuous = cplex.SparsePair(ind = [x[i, j, k] - x[j, t, k] for i in range(N + 1) for t in range(1, N + 2)], val = [1.0] * (N + 1) * (N + 1))
    #         model.linear_constraints.add(lin_expr = [continuous], senses = "E", rhs = [0])
    ...
    if solution:
        ...
        for k in range(K):
            ...
            while current_point != N + 1:
                for i in range(N + 2):
                    if model.solution.get_values(x[current_point, i, k]) >= 0.9:
                        current_point = i
